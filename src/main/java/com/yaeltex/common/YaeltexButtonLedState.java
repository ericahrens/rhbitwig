...
public class YaeltexButtonLedState extends InternalHardwareLightState {
    ...
    public static YaeltexButtonLedState of(final double r, final double g, final double b) {
        final int rv = (int) Math.floor(r * 255);
        final int gv = (int) Math.floor(g * 255);
        final int bv = (int) Math.floor(b * 255);
        final int color = ColorUtil.matchToIndex(r, g, b);
        //SeqArp168Extension.println(" %02x %02x %02x => %d", rv, gv, bv, color);
        return YaeltexButtonLedState.of(color);
    }
