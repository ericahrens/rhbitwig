...
public class AllenHeathK2ControllerExtension extends ControllerExtension {
	...
	public void init() {
		...
		viewControl = new ViewCursorControl(host, controlList);
	...
	private void initDocumentProperties() {
		...
		for (int i = 0; i < 8; i++) {
			...
			padAssignment.addValueObserver(value -> {
				assignmentList[index].assign(value, drumPadsList);
			});
		}
	}
	...
	private void initSendsButtons() {
		...
		for (int i = 0; i < 8; i++) {
			...
			delButton.bind(mainLayer, () -> {
				assignment.toggleSendValue(0);
			}, () -> assignment.sendStatusColor(0));
...
//		final int type = msg.getStatusByte() & 0xF0;
//		if (type == 144) {
//			RemoteConsole.out.println("NOTE {}", msg.getData1());
//			midiOut.sendMidi(msg.getStatusByte(), msg.getData1() + 72, 127);
//		} else if (type == 128) {
//			midiOut.sendMidi(msg.getStatusByte(), msg.getData1(), 0);
//		}
	}
