...
public class SequencerLayer extends Layer implements DrumSequencerSource.ChangeListener {
    ...
    private final int drumPadOffset = 36;
    ...
    public SequencerLayer(final Layers layers, final HwElements hwElements, final ViewCursorControl viewCursorControl,
        final PadGrouping padGrouping) {
        ...
        for (int i = 0; i < 8; i++) {
            final int index = i;
            final MultiStateHardwareLight light = hwElements.getChannelLight(i);
            bindLightState(() -> this.playingState(index), light);
        }
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 8; j++) {
                final int columnIndex = j;
                final int stepIndex = i * 8 + j;
                final StateButton button = hwElements.getStateButton(i, j);
                button.bindPressed(
                    this, () -> {
        ...
        GlobalContext.getContext().registerForListener(
            DrumSequencerSource.class, LISTENER_ID, this::connectToSequencer,
    ...
    private void connectToSequencer(final DrumSequencerSource source) {
        AllenHeathK2ControllerExtension.println(
            " Connected to Drum Sequencer gr=%f  gp=%f", source.getGridResolution(),
    ...
    @Override
    public void changePadsOffset(final int bankOffset, final int padOffset, final int bankSize) {
        this.selectedPadIndex = padOffset;
        adjustPosition();
    }
    
    @Override
    public void changeGridResolution(final double resolution) {
        AllenHeathK2ControllerExtension.println(" GRID Resolution = %f", resolution);
        cursorClip.setStepSize(resolution);
    }
    
    private void handlePressedStep(final int stepIndex) {
        final NoteStep note = assignments[stepIndex];
        if (note == null || note.state() == NoteStep.State.Empty) {
            cursorClip.setStep(
                stepIndex, 0, drumSequencerSource.getRefVelocity(),
    ...
    private RedGreenButtonState getStepLight(final int stepIndex) {
    ...
    private void handleNotePlaying(final PlayingNote[] notes) {
        ...
        for (final PlayingNote playingNote : notes) {
            ...
            if (baseIndex >= 0 && baseIndex < 16) {
                final int slotIndex = padGrouping.padIndexToSlot(baseIndex);
    ...
    private RedGreenButtonState playingState(final int trackIndex) {
        return playing[trackIndex] ? RedGreenButtonState.GREEN : RedGreenButtonState.OFF;
    }
    
    private void adjustPosition() {
        final int position = drumPadOffset + selectedPadIndex;
